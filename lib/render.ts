...
/** The stored wordCount on chapters and blog posts, and the bio length the
 *  thin-page gate reads. countWords() rather than a local split: the live counter
 *  in the admin (components/payload/fields/WordCount.tsx) shows an editor the
 *  same number this writes, through the same convertLexicalToPlaintext. */
function plainWordCount(data: Rich): number {
  if (!data) return 0;
  return countWords(convertLexicalToPlaintext({ data }));
}
...
/** A book that a published list or post names but that has no page on the site,
 *  reduced to the same BookSummary every row renderer already reads.
 *
 *  THE INPUT IS A DRAFT, so almost every field is missing by definition and this
 *  function's whole job is choosing what "missing" renders as:
 *
 *  - `rightsTier` falls back to `in-copyright`, the one tier that forbids
 *    delivery. A book whose tier nobody has chosen yet must never be treated as
 *    downloadable, and countFreePdfs() reads exactly this field to decide what a
 *    page TITLE claims.
 *  - `pdfUrl`, `rokomariUrl` and `priceTaka` are null, so the row offers no
 *    download and no buy button. Not an oversight: a draft book's price and
 *    affiliate link are unreviewed by definition, and an affiliate link to an
 *    unchecked URL is the kind of thing a reader remembers.
 *  - `ratingAverage` is null and `ratingCount` 0, which is already true: reviews
 *    attach to a published book page, and this book has none.
 *  - `slug` is whatever the draft has, possibly the empty string. Nothing links
 *    to it — `pending` is what stops that — so an empty slug is safe here, but it
 *    is NOT safe as a React key, which is why the list page keys on position. */
export function toPendingBookSummary(doc: PayloadBook): BookSummary {
  return {
    slug: typeof doc.slug === "string" ? doc.slug : "",
    title: doc.title,
    titleLatin: doc.titleLatin || null,
    rightsTier: isRightsTier(doc.rightsTier) ? doc.rightsTier : "in-copyright",
    authors: (doc.authors ?? [])
      .map((a) => entityRef(populated<PayloadAuthor>(a)))
      .filter((a): a is EntityRef => Boolean(a)),
    coverImage: mediaUrl(doc.cover) || DEFAULT_COVER_IMAGE,
    summary: "",
    pdfUrl: null,
    rokomariUrl: null,
    priceTaka: null,
    ...
    ratingCount: 0,
    featured: false,
    publishDate: new Date(doc.publishDate ?? doc.updatedAt ?? Date.now()).toISOString(),
  };
}

/** List entries embed a book relationship each; the note is per-entry
 *  editorial prose ("WHY this book is on the list").
 *
 *  An entry survives whenever `resolveBook` can answer at all — as a live book or
 *  as a pending one (see ReferencedBook in lib/types.ts). It is dropped only when
 *  the relationship resolves to nothing: the book was deleted, or the field is
 *  empty. That is a hole nobody can fill, so it is not rendered as one. */
export async function toListContent(
  l: PayloadList,
  toHtml: RichTextRenderer,
  resolveBook: (bookId: number) => ReferencedBook | undefined
): Promise<ListContent> {
  const entries: ListContent["entries"] = [];
  for (const entry of l.entries ?? []) {
    const id = relationshipId(entry.book);
    const ref = id === undefined ? undefined : resolveBook(id);
    if (!ref) continue; // deleted, or an empty relationship — not a hole to paint
    entries.push({ ...ref, noteHtml: await toHtml(asRich(entry.note)) });
  }
...
export async function toBlogPostContent(
  ...
  // Pending books stay in the rail, in the order the field lists them. The rail
  // is explicitly an unordered set (see the note at its render site), so unlike a
  // তালিকা there is no ranking for a stub to distort.
  const books = (post.relatedBooks ?? [])
    .map((raw) => {
      const id = relationshipId(raw);
      return id === undefined ? undefined : resolveBook(id);
    })
    .filter((b): b is ReferencedBook => Boolean(b));
