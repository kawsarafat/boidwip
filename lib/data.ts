...
import { tierAllowsOnlineReading, toBookSummary } from "@/lib/types";
import {
...
async function buildStore(): Promise<Store> {
  ...
  const [
    ...
    payload.find({ collection: "pages", pagination: false, depth: 1 }),
  ...
  const resolveBook = (id: number) => booksById.get(id);
