...
/** The visible words of a sanitized HTML fragment, as one collapsed line.
 *
 *  For the two things call sites do with a descriptionHtml they cannot render in
 *  full: count its words (the thin-page gate on /publisher and in the sitemap
 *  both did this inline, in two copies that had already drifted apart in
 *  whitespace handling) and cut an excerpt for a hub card.
 *
 *  A REGEX, DELIBERATELY, and only safe because of where the input comes from:
 *  every *Html field on these models was produced by lib/render.ts and
 *  sanitized there, so this is not parsing hostile markup — it is stripping tags
 *  from markup this codebase wrote. Tags become a SPACE rather than nothing, so
 *  `<p>এক</p><p>দুই</p>` counts two words instead of one run-together token. */
...
/** Words in a run of plain text, on one definition for the whole project.
 *
 *  Three places count words and they must all agree: the thin-page noindex gate
 *  (via htmlWordCount below), the stored wordCount on chapters and blog posts
 *  (via plainWordCount in lib/render.ts, which converts Lexical to text first),
 *  and the live counter an editor watches while typing. When those drift, an
 *  editor writes to a number the build does not use, and the page they were told
 *  is long enough gets a noindex anyway.
 *
 *  Whitespace-delimited, which is what makes it safe on Bengali: the script has
 *  no case, no apostrophes and no hyphenation rules worth special-casing, and
 *  its words are separated by ordinary spaces. \s+ covers the ZWNJ-free case;
 *  U+200C/U+200D join *within* a word and must not split it, which is exactly
 *  what \s+ does (they are not whitespace). */
export function countWords(text: string | null | undefined): number {
  if (!text) return 0;
  return text.trim().split(/\s+/).filter(Boolean).length;
}

/** Word count of a sanitized HTML fragment. Drives the thin-page noindex gate,
 *  so it must agree between the page that sets the meta tag and the sitemap that
 *  decides whether to advertise the URL — which is why it is one function. */
...
/** Where each slugged collection's documents live in the public URL space.
 *
 *  THE OTHER HALF OF THE LIST ABOVE. RESERVED_ROUTE_SLUGS says which segments
 *  the app owns; this says which collection owns which one. Both are the same
 *  fact about the URL space, so they live in the same file — and this file is
 *  importable from a client component, which is what makes one map serve all
 *  three callers: lib/payload/slugHistory.ts (writing redirects on rename), the
 *  admin's live-link column (a "use client" cell), and anything else that has a
 *  collection slug and needs a URL.
 *
 *  A collection absent from this map has no public single-document URL. Kept
 *  explicit rather than derived because there is no convention: `pages` sit at
 *  the site root while everything else is namespaced.
 *
 *  DELIBERATELY ABSENT: book-chapters. A chapter's URL is
 *  /book/<book-slug>/read/<chapter-slug>, which needs the parent book's slug —
 *  not something either caller has from the chapter alone. */
export const COLLECTION_URL_PREFIXES: Record<string, string> = {
  books: "/book",
  authors: "/author",
...
/** The slug shape every collection in this project enforces on save. Repeated
 *  here rather than imported from a collection config: this module must stay
 *  free of Payload imports so a client component can use it. */
const PUBLIC_SLUG_PATTERN = /^[a-z0-9-]+$/;

/** The public path of one document, or null when it has none — either because
 *  its collection is not in the map above or because the slug could never have
 *  been a URL segment. Returns a PATH, not an absolute URL: the two callers
 *  differ on origin (a build-time absolute URL, a browser-relative anchor). */
export function publicPathFor(
  collectionSlug: string | undefined,
  slug: string | undefined,
): string | null {
  if (!collectionSlug || !slug) return null;
  const prefix = COLLECTION_URL_PREFIXES[collectionSlug];
...
/** A book AS A LIST OR POST REFERENCES IT, which is not the same thing as a book
 *  that has a page.
 *
 *  WHY THIS DISTINCTION EXISTS. Writing a list is one job and detailing a book
 *  is another, and requiring the second before the first can be published is
 *  what stalls lists: an editor with a ranking of ten books in mind had to write
 *  ten synopses, pick ten rights tiers and clear ten covers before the তালিকা
 *  could go live. So a list may now name a book that is still a draft — title
 *  typed, nothing else — and publish anyway. The draft books surface in the
 *  admin dashboard's "Needs attention" list (see the referenced-drafts metric in
 *  components/payload/dashboard/queries.ts) and can be filled in later, or never.
 *
 *  WHAT `pending` COSTS, stated plainly because it is a real trade and not a
 *  detail: a pending entry publishes the little the draft knows about itself —
 *  its title, its authors, its cover — on the live list page, while the book
 *  itself stays unpublished. That is the point of the feature rather than a leak,
 *  but it means a draft book's title is not private once a published list names
 *  it.
 *
 *  WHY THE BOOK IS STILL A FULL BookSummary rather than some narrower
 *  "pending book" shape: everything that paints a book row already reads a
 *  BookSummary, and a second shape would need a second renderer, which is how
 *  the two drift. lib/render.ts fills the fields a draft cannot answer with the
 *  same neutral values a real book with nothing entered would have — no PDF, no
 *  price, no rating — so a pending row degrades rather than lying. */
export interface ReferencedBook {
  book: BookSummary;
  /** True when the referenced book has no page on the site: still a draft, or
   *  published with a future date. The row must not link anywhere, because this
   *  site is a build artefact and an unbuilt URL 404s rather than degrading. */
  pending: boolean;
}

export interface ListEntry extends ReferencedBook {
  /** Sanitized HTML: the editor's reason this book is on the list. */
  noteHtml: string;
}

/** The id behind a Payload relationship value, whatever depth it arrived at: a
 *  bare number at depth 0, the populated document at depth 1+.
 *
 *  One copy because four callers need the same answer — the two list/post
 *  builders in lib/render.ts, the pending-book resolution in lib/data.ts, and
 *  the dashboard metric that counts referenced drafts — and a hand-inlined
 *  `typeof raw === "object" ? raw.id : raw` in each is how one of them ends up
 *  handling only one of the two shapes. Returns undefined rather than throwing
 *  for anything else, including the null Payload writes for a relationship whose
...
export interface BlogPostContent {
  ...
  readingTimeMinutes: number;
  /** Books referenced by the post, for the "এই লেখায় যে বইগুলো" rail. Each
   *  carries a `pending` flag — see ReferencedBook. */
