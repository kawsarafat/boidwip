 ...
 *
 *  The collection's read access returns approved-only for anonymous requests
 *  (collections/Reviews.ts), so this query cannot surface pending or spam
 *  rows, and authorEmail/ipHash are stripped by field-level access before
 *  they leave the server. The fetch filters on the relationship's SLUG
...
export default function ReviewSection({ bookSlug }: { bookSlug: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?${query.toString()}`, { headers: { accept: "application/json" } })
