...
import { SITE_NAME, LIST_SUBJECT } from "@/lib/seo";
import SearchBox from "@/components/SearchBox";
import SiteMark from "@/components/SiteMark";

/* The drawer's own copy of the header strip's page-type links. LIST_SUBJECT
   rather than a literal "তালিকা": the hub's h1, its <title>, the footer link
   and this row are one decision, and lib/seo.ts owns the wording. The drawer
   has room for the full phrase where the desktop strip does not. */
const BROWSE_LINKS = [
  { href: "/new", label: "নতুন বই" },
  { href: "/popular", label: "জনপ্রিয় বই" },
  { href: "/list", label: LIST_SUBJECT },
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4">
          ...
          <ul className="mt-2 space-y-1">
            ...
            {/* The subject index, at the foot of the subjects it indexes. The
                drawer lists every category rather than a top-N slice, so this is
                not an overflow link like the desktop strip's — it is simply the
                only path to /category on a phone that does not go through the
                footer. Accent and text-only, the same treatment the footer gives
                the same link, so the two cannot read as different destinations. */}
            <li>
              <Link
                href="/category"
                onClick={close}
                className="flex min-h-11 items-center rounded-lg px-3 py-2.5 text-sm font-semibold text-accent transition active:bg-surface-sunken hover:bg-surface-sunken"
              >
                সব বিষয়
              </Link>
            </li>
