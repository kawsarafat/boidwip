...
export default function TableOfContents({
  items,
  label = "এই অধ্যায়ে যা আছে",
  navLabel = "অধ্যায়ের সূচিপত্র",
}: {
  items: TocEntry[];
  /** The summary line. Defaults to the chapter wording, which is what this
   *  component was written for; a blog post says "এই লেখায়" instead, and calling
   *  a post's sections "অধ্যায়" is the kind of copy slip a reader notices before
   *  anything else on the page. */
  label?: string;
  /** Accessible name for the <nav>, kept in step with `label` rather than left
   *  saying "chapter" while the visible heading says something else. */
  navLabel?: string;
}) {
  ...
  return (
    <details className="group card my-6 overflow-hidden">
      ...
      <nav aria-label={navLabel} className="border-t border-rule px-4 py-3 sm:px-5">
