...
import { isReservedRouteSlug, tierAllowsOnlineReading } from "@/lib/types";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const publisherRoutes: MetadataRoute.Sitemap = publishers
    .filter((p) => {
      const descWords = p.descriptionHtml
        ? p.descriptionHtml.replace(/<[^>]+>/g, " ").split(/\s+/).filter(Boolean).length
        : 0;
      return !isThinEntityPage(p.bookCount, descWords);
    })
