...
import { toBookSummary, countFreePdfs, isSafeHttpUrl, htmlWordCount } from "@/lib/types";
import {
  publisherTitle,
  publisherHeading,
...
export async function generateMetadata({
  ...
  //
  // htmlWordCount rather than a regex here: the sitemap holds this URL out under
  // the SAME predicate, and the two were separate inline copies that had already
  // drifted in how they treated whitespace. A page saying noindex while the
  // sitemap advertises it is the mixed signal both lines exist to avoid.
  const descWords = htmlWordCount(publisher.descriptionHtml);
  return {
    title,
    description,
      ...
      },
      {
        ...
        itemListElement: [
          { "@type": "ListItem", position: 1, name: "হোম", item: SITE_URL },
          {
            ...
            item: `${SITE_URL}/publisher`,
          },
          { "@type": "ListItem", position: 3, name: publisher.name, item: url },
        ],
      },
      ...(books.length > 0
        ? [
  ...
  return (
    <div className="shell py-6 sm:py-8">
      ...
      <Breadcrumb
        items={[
          { label: "হোম", href: "/" },
          { label: PUBLISHER_INDEX_SUBJECT, href: "/publisher" },
