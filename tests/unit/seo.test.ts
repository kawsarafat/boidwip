...
describe("bookTitle / bookH1", () => {
  ...
  it("leads a no-download title with what the page does offer", () => {
    // The keywords sit between the title and the byline on purpose: the byline
    // is the part that can be truncated without losing a query. The layout
    // appends " | বইদ্বীপ", so the brand must not appear here.
    expect(bookTitle(copyrightBook)).toBe(
      "নতুন উপন্যাস বইয়ের দাম, রিভিউ ও সারসংক্ষেপ - শরৎচন্দ্র চট্টোপাধ্যায়",
    );
    expect(bookTitle(copyrightBook)).not.toContain("বইদ্বীপ");
  });
  ...
  it("drops the byline cleanly when a book has no author", () => {
    ...
    expect(bookTitle({ ...anon, pdfUrl: null })).toBe("দেবদাস বইয়ের দাম, রিভিউ ও সারসংক্ষেপ");
  });
});
